...
if [ ${HARBORHOSTNAME} ]; then sed -i '/hostname = harbor.domain.com/c\\hostname = '${HARBORHOSTNAME}'' harbor.cfg; fi
...
if [ ${HARBOR_ADMIN_PASSWORD} ]; then sed -i '/harbor_admin_password = password/c\\harbor_admin_password = '${HARBOR_ADMIN_PASSWORD}'' harbor.cfg; fi
if [ ${AUTH_MODE} ]; then sed -i '/auth_mode = auth/c\\auth_mode = '${AUTH_MODE}'' harbor.cfg; fi
if [ ${LDAP_URL} ]; then sed -i '/ldap_url = url/c\\ldap_url = '${LDAP_URL}'' harbor.cfg; fi
if [ ${LDAP_SEARCHDN} ]; then sed -i '/ldap_searchdn = admin/c\\ldap_searchdn = '${LDAP_SEARCHDN}'' harbor.cfg; fi
if [ ${LDAP_SEARCH_PWD} ]; then sed -i '/ldap_search_pwd = password/c\\ldap_search_pwd = '${LDAP_SEARCH_PWD}'' harbor.cfg; fi
if [ ${LDAP_BASEDN} ]; then sed -i '/ldap_basedn = dc/c\\ldap_basedn = '${LDAP_BASEDN}'' harbor.cfg; fi
if [ ${LDAP_FILTER} ]; then sed -i '/ldap_filter = (objectClass)/c\\ldap_filter = ('${LDAP_FILTER}')' harbor.cfg; fi
